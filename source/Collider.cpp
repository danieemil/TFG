...
Collider::Collider(const Vector2d<float>& pos, Shape* s, const CollisionFlag& ft, const CollisionFlag& fi, const CollisionType& t, void* c, Callback cb, int i, float a, const Vector2d<float>& rot_cent, const Vector2d<float>& vel)
: position(pos), previous_position(pos), type_flags(ft), interested_flags(fi), type(t), creator(c), callback(cb), index(i), angle(a), rotation_center(rot_cent), velocity(vel)
{
    if(s!=nullptr)
    {
        addShape(s);
...
: position(c.position), previous_position(c.previous_position), bounds(c.bounds), type_flags(c.type_flags), interested_flags(c.interested_flags), creator(c.creator), callback(c.callback), index(-1), angle(c.angle), rotation_center(c.rotation_center), velocity(c.velocity)
{
